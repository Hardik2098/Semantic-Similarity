...
#to use tensorflow metrics in keras
...
# self created function to calculate precision and recall at different thresholds
...
class SiameseBiLSTM:
    ...
    def train_model(self, sentences_pair, is_similar, embedding_meta_data, epochs, threshold, model_save_directory='./'):
        """
        ...
        val_data_x1, val_data_x2, val_labels, leaks_val = create_train_dev_set(tokenizer, sentences_pair, is_similar, self.max_sequence_length,
        ...
        model.compile(loss='binary_crossentropy', optimizer='adam',
                      # metrics=['acc', as_keras_metric(tf.metrics.precision), as_keras_metric(tf.metrics.recall)]
                      metrics = ['acc' ,precision_threshold(threshold), recall_threshold(threshold)]
        )
        ...
        checkpoint_dir = model_save_directory + 'checkpoints/' + STAMP + '/'
        ...
        return bst_model_path, model, history


    def update_model(self, saved_model_path, new_sentences_pair, is_similar, embedding_meta_data, threshold, epochs):
        """
        ...
        val_data_x1, val_data_x2, val_labels, leaks_val = create_train_dev_set(tokenizer, new_sentences_pair,
                                                                               ...
                                                                               self.validation_split_ratio)
        model = load_model(saved_model_path, custom_objects = {'precision' : precision_threshold(threshold), 'recall':recall_threshold(threshold)})
        model_file_name = saved_model_path.split('/')[-1]
        STAMP = 'lstm_%d_%d_%.2f_%.2f' % (self.number_lstm_units, self.number_dense_units, self.rate_drop_lstm, self.rate_drop_dense)
        string = ''
        for i in saved_model_path.split('/')[:-2]:
            string += i + '/'
        new_model_checkpoint_path  = str(string) + STAMP + '/' 
        # print (new_model_checkpoint_path)
        new_model_path = new_model_checkpoint_path + model_file_name
        # print (new_model_path)
        model_checkpoint = ModelCheckpoint(new_model_path,
        ...
        history = model.fit([train_data_x1, train_data_x2, leaks_train], train_labels,
                  validation_data=([val_data_x1, val_data_x2, leaks_val], val_labels),
                  epochs=epochs, batch_size=3, shuffle=True,
                  # callbacks=[model_checkpoint, tensorboard]
                  )

        return new_model_path, model, history
